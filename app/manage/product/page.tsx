  ...
  const handleDelete = async (id: string) => {
    try {
      const result = await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/product?_id=${id}`
      );
      if (result.status === 200) {
        Swal.fire({
    ...
    <>
      ...
      <div className="flex flex-col justify-center items-center">
        <nav className="flex w-full justify-around items-center p-5 gap-5">
          <Link
            href="/manage/product"
            className="bg-sky-200 p-5 rounded w-full text-center"
          >
            Manage Product
          </Link>
          <Link
            href="/manage/blogs"
            className="bg-amber-200 p-5 rounded w-full text-center"
          >
            Manage Blogs
          </Link>
          <button
            className="w-full bg-green-200 p-5 rounded"
            onClick={() => SetcreateStatus((prev) => !prev)}
          >
            Create Product
          </button>
        </nav>
        {createStatus && (
          <>
            <form
              ...
              className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-20 bg-white border rounded shadow-lg gap-5"
            >
              ...
              <button
                type="submit"
                className="border w-full p-3 rounded bg-green-200"
              >
                Submit
              </button>
            </form>
            ...
            >
              ...
              <div className="flex justify-end gap-3">
                ...
                {editingId === item._id && (
                  <button
                    name="save"
                    className="rounded p-3 bg-green-200"
                    type="submit"
                  >
                    Save
                  </button>
                ...
                {editingId != item._id && (
                  <button
                    ...
                    className="border p-3 rounded bg-red-200"
                  >
                    Delete
                  </button>
                )}
